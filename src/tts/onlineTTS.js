...
// import TransWorker from ‘js / transcode.worker.js’ 引入了webWorker文件, webWorker相当于js中的线程，
// 在主线程中启动一个子线程不影响ui，，有关webworker内容如下：
// 在vite中不需要像webpack构建那样去配置vue.config.js文件，只需要改变webworker文件的引入方式：
// let myworker = new Worker(new URL('./js/processor.worker.js', import.meta.url))
let myworker = new Worker(new URL('./js/transcode.worker.js', import.meta.url))
  ...
  connectWebSocket () {
    ...
    return getWebsocketUrl().then(url => {
      ...
      if ('WebSocket' in window) {
        console.log('WebSocket 开始建立链接');
        ttsWS = new WebSocket(url)
      } else if ('MozWebSocket' in window) {
        console.log('MozWebSocket 开始建立链接');
...
// 作者：wangpq
// 链接：https://juejin.cn/post/7201019738809237562
// 来源：稀土掘金
